...
export const SlackTestMessageButton: React.FC<SlackTestMessageButtonProps> = ({
  ...
  const { t } = useI18n();
  const resolvedButtonText = buttonText ?? t("slack.sendTestMessage");
  // Test message mutation
  const testMessageMutation = api.slack.sendTestMessage.useMutation({
    onSuccess: (data) => {
      showSuccessToast({
        title: t("slack.testMessageSent"),
        description: t("slack.testMessageSentDescription"),
      });
      onSuccess?.(data.channelInfo);
    },
    onError: (error) => {
      showErrorToast(t("slack.failedToSendTestMessage"), error.message);
