...
export const createSelectedModelSchema = (
  translateText: TranslateText = defaultTranslateText,
) =>
  z.object({
    provider: z.string().min(1, translateText("Select a provider")),
    model: z.string().min(1, translateText("Select a model")),
...
const createCategoricalOptionSchema = (
  ...
  z.object({
    value: z.string().trim().min(1, translateText("Enter a category value")),
  });

export const createTemplateFormSchema = (
  translateText: TranslateText = defaultTranslateText,
) => {
  const categoricalOptionSchema = createCategoricalOptionSchema(translateText);

  return z
    .object({
      name: z.string().min(1, translateText("Enter a name")),
      type: z
      ...
      prompt: z
        ...
        }, translateText("Variables must only contain letters and underscores (_)")),

      variables: z.array(
        z
          .string()
          .min(1, translateText("Variables must have at least one character")),
      ),
    ...
    .superRefine((value, ctx) => {
      ...
      const isCategorical =
        value.scoreDataType === ScoreDataTypeEnum.CATEGORICAL;
      ...
      if (isCodeType && !value.sourceCode?.trim()) {
        ...
        ctx.addIssue({
          code: "custom",
          message: translateText("Enter {language} source code", {
      ...
      if (isLlmType) {
        if (!value.prompt?.trim()) {
          ctx.addIssue({
            code: "custom",
            message: translateText("Enter a prompt"),
            path: ["prompt"],
          });
        }
        if (!value.reasoningDescription?.trim()) {
          ctx.addIssue({
            code: "custom",
            message: translateText("Enter a reasoning function"),
        ...
        if (!value.scoreDescription?.trim()) {
          ctx.addIssue({
            code: "custom",
            message: translateText("Enter a score function"),
            ...
            ctx.addIssue({
              ...
              message:
                violation.type === "minimum_count"
                  ? translateText(getMinimumCategoricalCategoriesMessage())
                  : translateText("Categories must be unique"),
              path:
                violation.type === "minimum_count"
                  ? ["categories"]
...
const selectedModelSchema = createSelectedModelSchema();
const categoricalOptionSchema = createCategoricalOptionSchema();
