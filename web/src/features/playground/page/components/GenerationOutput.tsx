    ...
    output || outputToolCalls.length ? (
      <div className="absolute top-2 right-3 flex space-x-1 opacity-50">
        <Button
          ...
          title={translateText("Toggle Input/Output JSON")}
        >
          <BracesIcon size={15} />
        </Button>
        ...
        <Button
          ...
          title={translateText("Copy output")}
        >
        ...
        <Button
          ...
          title={translateText("Add as assistant message")}
          disabled={isAdded}
        >
          {isAdded ? checkIcon : plusIcon}
          <span className="text-xs">{translateText("Add to messages")}</span>
        </Button>
      </div>
    ) : null;
