...
export function BatchExportsTable(props: { projectId: string }) {
  ...
  const columns = [
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <div className="space-y-1">
                    <div>
                      {t("batchExports.created", {
                        date: new Date(createdAt).toLocaleString(),
                      })}
                    </div>
                    <div>
                      {t("batchExports.finished", {
                        date: finishedAt
    ...
    },
    {
      accessorKey: "url",
      id: "url",
      header: t("batchExports.downloadUrl"),
      size: 130,
      cell: (info) => {
    ...
    },
    {
      accessorKey: "format",
      id: "format",
      header: t("batchExports.format"),
      size: 70,
    },
    {
      accessorKey: "user",
      id: "user",
      header: t("batchExports.createdBy"),
      size: 150,
      cell: ({ row }) => {
          ...
          <div className="flex items-center space-x-2">
            <Avatar className="h-7 w-7">
              <AvatarImage
                ...
                alt={user?.name ?? t("batchExports.userAvatar")}
              />
            </Avatar>
            <span>{user?.name ?? t("batchExports.unknownUser")}</span>
          </div>
        );
      },
    },
        ...
        return (
          ...
          >
            <AlertDialogTrigger asChild>
              ...
              >
                {t("batchExports.cancel")}
              </ActionButton>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  {t("batchExports.cancelTitle")}
                </AlertDialogTitle>
                <AlertDialogDescription>
                  {t("batchExports.cancelDescription")}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>{t("batchExports.keep")}</AlertDialogCancel>
                <AlertDialogAction
                  onClick={() => {
