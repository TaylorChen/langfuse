...
export const NewPromptForm: React.FC<NewPromptFormProps> = (props) => {
  ...
  const form = useForm({
    resolver: zodResolver(createNewPromptFormSchema(translateText)),
    mode: "onTouched",
    defaultValues,
  });
  ...
  return (
    <Form {...form}>
      ...
      >
              ...
              return (
                <div>
                  <FormItem>
                    ...
                    <FormDescription>
                      {t("prompts.nameDescriptionPrefix")}{" "}
                      <a
                        target="_blank"
                    ...
                    {form.getFieldState("name").error ? (
                      <div className="text-destructive flex flex-row space-x-1 text-sm font-medium">
                        ...
                        {errorMessage?.includes("already exist") ? (
                          ...
                          >
                            {t("prompts.createNewVersionForIt")}
        ...
        <>
          <FormItem>
            <FormLabel>{t("prompts.promptLabel")}</FormLabel>
            <FormDescription>
              {t("prompts.promptDescriptionPrefix")}{" "}
              <code className="text-xs">{"{{variable}}"}</code>{" "}
              {` ${t("prompts.promptDescriptionMiddle")} `}
              <b className="font-semibold"> {t("prompts.note")}</b>{" "}
              {t("prompts.promptDescriptionSuffix")}
            </FormDescription>
            <Tabs
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("prompts.configLabel")}</FormLabel>
              <FormDescription>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="relative">
              ...
              <FormDescription>
                {t("prompts.commitMessageDescription")}
              </FormDescription>
              <FormControl>
                <Textarea
        ...
        {initialPrompt ? (
          <div className="flex flex-col gap-2">
            ...
            <Button
              ...
              disabled={!form.formState.isValid}
            >
              {t("prompts.saveNewPromptVersion")}
            </Button>
          </div>
        ) : (
          <Button
