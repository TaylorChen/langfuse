...
export default function RemapEvaluatorPage() {
  ...
  const updateJobMutation = api.evals.updateEvalJob.useMutation({
    ...
    onError: (err) => {
      setError(
        err.message ?? translateText("Failed to update old eval configuration"),
      );
    },
  });
  ...
  const deleteJobMutation = api.evals.deleteEvalJob.useMutation({
    ...
    onError: (err) => {
      setError(
        err.message ?? translateText("Failed to delete old eval configuration"),
      );
    },
  });
  ...
  return (
    <Page
      ...
      scrollable
      headerProps={{
        title: translateText("Upgrade Evaluator"),
        breadcrumb: [
          {
      ...
      <div className="space-y-4">
        <div>
          <p className="text-muted-foreground text-sm">
            ...
            >
              {translateText("Follow our step-by-step guide")}
            </a>{" "}
            {translateText("to upgrade successfully.")}
          </p>
          {mappedConfig ? (
            <Alert
              variant="default"
            ...
            <div className="grid grid-cols-[1fr_2px_1fr] items-start">
              ...
              <div className="space-y-4 p-3">
                <div className="flex items-center gap-2 pb-2">
                  <h3 className="text-lg font-semibold">
                    {translateText("Legacy Configuration")}{" "}
                    {isTraceTarget(oldConfig.targetObject)
                      ? translateText("(runs on traces)")
                      : ""}
                  </h3>
              ...
              <div className="space-y-4 p-3">
                <h3 className="pb-2 text-lg font-semibold">
                  {translateText("New Configuration")}{" "}
                  {isTraceTarget(oldConfig.targetObject)
                    ? translateText("(runs on observations)")
                    : ""}
                </h3>
                <InnerEvaluatorForm
                  projectId={projectId}
                      ...
                      <div className="flex items-center">
                        ...
                        >
                          {legacyAction === "keep-active"
                            ? translateText("Save & keep legacy active")
                            : legacyAction === "mark-inactive"
                              ? translateText("Save & mark legacy inactive")
                              : translateText("Save & delete legacy")}
                        </Button>
                        <DropdownMenu>
