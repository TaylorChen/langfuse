...
export function EvaluatorSelectionStep(props: EvaluatorSelectionStepProps) {
  ...
  const getPromptPreview = (evaluator: Evaluator) => {
    if (isPreviewLoading) {
      return t("batchActions.loadingPreview");
    }
    ...
    if (!mappingResult.success) {
      return t("batchActions.invalidEvaluatorMapping");
    }
    ...
    return renderPromptPreviewFromObservation({
      ...
      emptyPromptMessage: t("batchActions.templateHasNoPrompt"),
    });
  };
      ...
      <div className="min-h-0 flex-1">
        {isQueryLoading ? (
          <p className="text-muted-foreground text-sm">
            {t("batchActions.loadingEvaluators")}
          </p>
        ) : isQueryError ? (
          <Card>
          ...
          <Card>
            <CardContent className="text-muted-foreground p-4 text-sm">
              {t("batchActions.noScopedEvaluators", {
                scope: t(
                  evaluatorScopeLabel === "observation"
