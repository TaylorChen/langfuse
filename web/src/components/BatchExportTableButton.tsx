...
export const BatchExportTableButton: React.FC<BatchExportTableButtonProps> = (
  ...
  const createExport = api.batchExport.create.useMutation({
    ...
    onSuccess: () => {
      showSuccessToast({
        title: t("batchExport.queuedTitle"),
        description: t("batchExport.queuedDescription"),
        duration: 10000,
        link: {
  ...
  const getWarningMessage = () => {
    switch (props.tableName) {
      case BatchTableNames.Traces:
        return t("batchExport.traceWarning");
      case BatchTableNames.Observations:
        return t("batchExport.observationWarning");
      case BatchTableNames.Events:
        return t("batchExport.eventWarning");
      case BatchTableNames.Sessions:
        return t("batchExport.sessionWarning");
      case BatchTableNames.AuditLogs:
        return t("batchExport.auditLogWarning");
      default:
      ...
      <DropdownMenuPortal>
        <DropdownMenuContent className="w-80">
          ...
          {Object.entries(exportOptions).map(([key, options]) => (
            ...
            >
              {t("batchExport.asFormat", { format: options.label })}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenuPortal>
