...
export function MembersTable({
  ...
  const columns: LangfuseColumnDef<MembersTableRow>[] = [
    ...
    },
    {
      ...
      headerTooltip: {
        description: t("members.organizationRoleTooltip"),
            ...
            {disableInProjectSettings && hasCudAccessOrgLevel ? (
              <HoverCard openDelay={0} closeDelay={0}>
                ...
                <HoverCardPortal>
                  ...
                  >
                    <p className="text-xs">
                      {t("members.organizationRoleEditPrefix")}{" "}
                      <Link
                        href={`/organization/${orgId}/settings/members`}
                        className="underline"
                      >
                        {t("members.organizationSettings")}
                      </Link>
                      .
                    </p>
                  </HoverCardContent>
                ...
                if (
                  confirm(
                    userId === session.data?.user?.id
                      ? t("members.confirmLeaveOrganization")
  ...
  return (
    <>
      <DataTableToolbar
        ...
        searchConfig={{
          metadataSearchFields: [translateText("Name"), translateText("Email")],
...
const OrgRoleDropdown = ({
  ...
  const mut = api.members.updateOrgMembership.useMutation({
    onSuccess: (data) => {
      ...
      if (data.userId === session.data?.user?.id) session.update();
      showSuccessToast({
        title: t("members.saved"),
        description: t("members.organizationRoleUpdated"),
        duration: 2000,
      });
    },
  });
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        if (
          userId !== session.data?.user?.id ||
          confirm(t("members.confirmChangeOwnOrganizationRole"))
        ) {
          mut.mutate({
            orgId,
            orgMembershipId,
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        if (
          userId !== session.data?.user?.id ||
          confirm(t("members.confirmChangeOwnProjectRole"))
        ) {
          mut.mutate({
            orgId,
            orgMembershipId,
