...
export const PromptModelStep: React.FC<PromptModelStepProps> = ({
  ...
  return (
    <div className="space-y-6">
      <StepHeader
        title={translateText("Prompt & Model Configuration")}
      ...
      <FormField
        ...
        render={() => (
          <FormItem>
            <FormLabel>{translateText("Prompt")}</FormLabel>
            <div className="mb-2 flex gap-2">
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {selectedPromptName || translateText("Select a prompt")}
                ...
                >
                  <InputCommand>
                    <InputCommandInput
                      placeholder={translateText("Search prompts...")}
                      className="h-9"
                      variant="bottom"
                    />
                    <InputCommandList>
                      <InputCommandEmpty>
                        {translateText("No prompt found.")}
                      </InputCommandEmpty>
                      <InputCommandGroup>
                        {promptsByName &&
              ...
              <Popover>
                ...
                >
                  <InputCommand>
                    <InputCommandInput
                      placeholder={translateText("Search versions...")}
                      className="h-9"
                    />
                    <InputCommandList>
                      <InputCommandEmpty>
                        {translateText("No version found.")}
                      </InputCommandEmpty>
                      <InputCommandGroup className="overflow-y-auto">
                        ...
                        promptsByName[selectedPromptName] ? (
                            ...
                            >
                              <div className="flex min-w-0 flex-1 items-center gap-2">
                                <span className="shrink-0">
                                  {translateText("Version")} {prompt.version}
                                </span>
                          ...
                          <InputCommandItem disabled>
                            {translateText("No versions available")}
                          </InputCommandItem>
                        )}
                      </InputCommandGroup>
                    </InputCommandList>
      ...
      <FormField
        ...
        render={() => (
          <FormItem>
            <FormLabel>{translateText("Model")}</FormLabel>
            <ModelParameters
              {...{
                modelParams,
                availableModels,
        ...
        render={({ field }) => (
          <FormItem>
            <div className="flex items-center justify-between">
              <FormLabel>
                {translateText("Structured output (optional)")}
              </FormLabel>
              <Switch
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      >
                        <InputCommand>
                          <InputCommandInput
                            placeholder={translateText("Search schemas...")}
                            className="h-9"
                            variant="bottom"
                          />
                          <InputCommandList>
                            <InputCommandEmpty>
                              {translateText("No schema found.")}
                            </InputCommandEmpty>
                            <InputCommandGroup>
            ...
            <CardDescription>
              {structuredOutputEnabled
                ...
                : translateText("Enable to enforce a specific output format")}
            </CardDescription>
            <FormMessage />
          </FormItem>
        )}
