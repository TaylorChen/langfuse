  ...
  if (!isOpen) {
    return (
      <Button
        ...
        onClick={openSearch}
        aria-label={translateText("Find in messages")}
      >
        <Search className="h-3.5 w-3.5" />
        <span className="hidden lg:inline">{translateText("Find")}</span>
      </Button>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        placeholder={translateText("Find in messages")}
      ...
      <IconButton
        icon={ChevronUp}
        label={translateText("Previous result")}
        onClick={previousMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={ChevronDown}
        label={translateText("Next result")}
        onClick={nextMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={X}
        label={translateText("Close search")}
        onClick={closeSearch}
      />
    </div>
  );
