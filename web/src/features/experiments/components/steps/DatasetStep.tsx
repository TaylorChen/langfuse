...
export const DatasetStep: React.FC<DatasetStepProps> = ({
  ...
  return (
    <div className="space-y-6">
      <StepHeader
        title={translateText("Dataset Selection")}
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{translateText("Dataset")}</FormLabel>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="h-8">
                      {translateText("Expected columns")}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80">
                    <div className="space-y-2">
                      <h4 className="leading-none font-medium">
                        {translateText("Expected Dataset Structure")}
                      </h4>
                      <p className="text-muted-foreground text-sm">
                        {translateText("Based on prompt")} {selectedPromptName}{" "}
                        v{selectedPromptVersion}
                      </p>
                      <div className="space-y-1 pt-2">
                        <p className="text-sm font-medium">
                          {translateText("Input variables:")}
                        </p>
      ...
      {selectedDatasetId && datasetVersions && datasetVersions.length > 0 && (
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {translateText("Dataset Version (Optional)")}
              </FormLabel>
              <Select
                onValueChange={(value) => {
                  if (value === "latest") {
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={translateText("Latest version")}
                    />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
                  <SelectItem value="latest">
                    {translateText("Latest version (default)")}
                  </SelectItem>
            ...
            <Card className="border-dark-yellow bg-light-yellow relative overflow-hidden rounded-md shadow-none group-data-[collapsible=icon]:hidden">
              <CardHeader className="p-2">
                <CardTitle className="text-dark-yellow flex items-center justify-between text-sm">
                  <span>{translateText("Invalid configuration")}</span>
            ...
            <Card className="border-dark-green bg-light-green relative overflow-hidden rounded-md shadow-none group-data-[collapsible=icon]:hidden">
              <CardHeader className="p-2">
                <CardTitle className="text-dark-green flex items-center justify-between text-sm">
                  <span>{translateText("Valid configuration")}</span>
