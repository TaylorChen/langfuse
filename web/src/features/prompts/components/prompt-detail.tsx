...
export const PromptDetail = ({
  ...
  const handleExperimentSuccess = async (data?: {
    ...
    showSuccessToast({
      title: t("prompts.experimentTriggeredTitle"),
      description: t("prompts.experimentTriggeredDescription"),
      link: {
  ...
  return (
    <Page
      headerProps={{
        title: prompt.name,
        titleTooltip: t("prompts.nameImmutableTooltip"),
        itemType: "PROMPT",
        help: {
          ...
          >
            <TabsBarList className="max-w-full min-w-0 justify-start overflow-x-auto">
              ...
              <TabsBarTrigger value="linked-generations">
                {t("prompts.linkedGenerationsTab")}
              </TabsBarTrigger>
              <TabsBarTrigger value="use-prompt">
                {t("prompts.usePromptTab")}
              </TabsBarTrigger>
            </TabsBarList>
            <TabsBarContent
                  ...
                  {prompt.type === PromptType.Chat && chatMessages ? (
                    ...
                    promptGraph.data?.resolvedPrompt ? (
                      <CodeView
                        ...
                        title={t("prompts.textPromptResolved")}
                      />
                    ) : (
                      <CodeView
                        content={renderRichPromptContent(prompt.prompt)}
                        originalContent={prompt.prompt}
                        title={t("prompts.textPrompt")}
                      />
                    )
                  ) : (
                    <JSONView
                      json={prompt.prompt}
                      title={t("prompts.promptTab")}
                    />
                  )}
                </PromptReferenceProvider>
            ...
            >
              <div className="flex h-full min-h-0 w-full flex-col gap-2 overflow-y-auto pb-4">
                ...
                <p className="text-muted-foreground pl-1 text-xs">
                  ...
                  >
                    {t("prompts.documentation")}
                  </a>{" "}
                  {t("prompts.usePromptDocsSuffix")}
                </p>
              </div>
            </TabsBarContent>
          </TabsBar>
